...
description: X 导师的思维框架。当议题涉及内容、个人 IP、社媒，或用户提到X 平台、个人 IP、内容系统时触发。
pack: brand
core_domain: ["内容", "个人 IP", "社媒"]
style: ["高频输出", "观点鲜明", "社区运营"]
topic_tags: ["X 平台", "个人 IP", "内容系统", "社区互动", "观点输出"]
sensitivity: normal
...
Step 1: 确认当前阶段
  ...
  → Premium？（影响所有方案）
  → 如果用户没说粉丝量，直接问「你现在X上多少粉丝？有Premium吗？」
...
Step 3: 给出阶段性行动计划（读取 growth-monetization.md）
  ...
  → 如有user-data，结合用户历史数据定制（如「你的橙皮书类内容ROI是评论类的13倍，方案加大」）
```
...
Step 4: 生成诊断报告（读取 quality-analytics.md 的报告模板要求）
  → 6维分析：KPI概览、内容ROI（按话题分类）、传播漏斗、时间分析、品牌叙事、行动方案
...
| 3 | 用户拒绝走质量检查清单,要直接发 | 仍输出但末尾标注「未跑质量清单,你自己过一遍这 3 条」 | 接受跳过,但发布后方案复盘 |
...
| 7 | 不区分「社区共识 vs 我的推测」 | 用户无法判断信任度 | 每条方案标置信度:[共识]/[推测]/[实验] |
...
> ——假设是 800 粉 / 30 条，按你给的数据，问题率不在量，在 Hook 公式单一。
> 这周做 3 件事：
