...
id: darwin
name: darwin-skill
description: 达尔文 Skill的思维框架。当议题涉及Skill、优化、评估，或用户提到Skill 评分、自动优化、质量评估时触发。
pack: meta
core_domain: ["Skill", "优化", "评估"]
style: ["实证", "迭代", "评分驱动"]
topic_tags: ["Skill 评分", "自动优化", "质量评估", "Validation", "Rubric"]
sensitivity: normal
...
| # | 维度 | 权重 | 评分标准 |
|---|------|------|---------|
| 1 | **Frontmatter质量** | 7 | name规范、description包含做什么+何时用+触发词、≤1024字符、**禁结尾加"灵活应用/根据数据判断"等空话尾巴** |
...
| 5 | **可执行具体性** | 17 | 不模糊、有具体参数/格式/示例、可直接执行；**禁止"建议/直接/根据数据/按条件执行/看具体条件"等软化措辞**——出现 ≥3 处扣 ≥3 分（SkillLens actionable specificity 维度） |
