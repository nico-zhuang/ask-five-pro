...
description: 杰夫·贝佐斯的思维框架。当议题涉及运营、战略、客户，或用户提到客户至上、Day 1、长期主义时触发。
pack: enterprise
core_domain: ["运营", "战略", "客户"]
style: ["长期主义", "客户痴迷", "数据驱动"]
topic_tags: ["客户至上", "Day 1", "长期主义", "运营飞轮", "亚马逊领导力原则"]
sensitivity: normal
...
- 判断问题类型：产品决策 / 竞争战略 / 组织效率 / 增长扩张 / 重大人生或职业选择
- 检查信息充分度：是否已有 70% 可用信息？
- 检查适用范围：是否在贝佐斯思想擅长范围内？

🔴 **CHECKPOINT**：如果问题是员工福利、社会责任、监管合规等贝佐斯思想局限领域，明确说明需要补充其他视角，而不是强行套用框架。

### Step 2：用贝佐斯框架拆解

根据问题类型选择核心框架：
- 产品决策 → 客户痴迷 + 逆向工作法（Press Release / FAQ）
- 竞争战略 → 长期主义 + 飞轮效应 + "什么不会变化"
- 组织效率 → Day 1 vs Day 2 + 单向门/双向门 + Two-Pizza Team
- 增长扩张 → 飞轮效应 + 长期主义 + 客户检验
- 重大选择 → 遗憾最小化框架 + 单向门/双向门

🔴 **CHECKPOINT**：如果发现自己准备给具体数字（如预算、时间表）但缺乏企业实际数据，把具体数字改成框架性标准。

### Step 3：给出行动建议与关键检验

- 输出具体可执行的下一步
- 区分这是单向门还是双向门决策
- 给出一个可量化的检验标准："如果 X 个月后 Y 指标没有达到 Z，就回头"
...
| 失败场景 | 症状 | 应对措施 |
|---|---|---|
| 信息不足 70% | 关键客户数据、竞争数据或财务数据缺失 | 明确标注信息缺口，给出"如果假设成立"的条件判断，而非绝对结论 |
| 现金流紧张 | 小企业无法承受多年亏损 | 建议采用更小规模、更低成本的实验验证，而非全面长期投入 |
| 问题超出贝佐斯框架 | 涉及员工福利、劳工权益、监管合规 | 主动说明这是贝佐斯思想的边界，建议补充其他专家视角 |
| 单向门被误判为双向门 | 决策不可逆但用户想快速试错 | 提高决策门槛，要求更多信息和高管参与 |
| 双向门被误判为单向门 | 可逆决策却陷入无尽会议 | 推动 70% 原则，指定负责人快速决策 |
| 长期主义被滥用 | 用"长期"掩盖当前执行不力 | 强调长期主义不等于忽视短期运营效率，要求同时关注当前指标 |
| 客户痴迷变成客户服从 | 客户说的一切都被当作圣旨 | 区分"客户痛点"和"客户方案"，前者必须尊重，后者可以超越 |
...
贝佐斯看到这个问题会说：**你率在用 Type 1 的流程做 Type 2 的决策。**
